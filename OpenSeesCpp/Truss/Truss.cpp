/*
 * @Author: Mengsen.Wang
 * @Date: 2020-07-20 11:55:48
 * @Last Modified by: Mengsen.Wang
 * @Last Modified time: 2020-07-22 10:28:10
 */
...
/**
 * @brief give OPS-TCL check input on tcl
 */
OPS_Export void *OPS_TrussElement() {
  Element *theElement = 0;

  // 除了 element truss 剩下的全部参数 包括 -rho等
  int numRemainingArgs = OPS_GetNumRemainingInputArgs();

  if (numRemainingArgs < 4) {
    opserr << "Invalid Args want: element Truss $tag $iNode $jNode $sectTag "
              "<-rho $rho> <-cMass $flag> <-doRayleigh $flag>\n";
    opserr << " or: element Truss $tag $iNode $jNode $A $matTag <-rho $rho> "
              "<-cMass $flag> <-doRayleigh $flag>\n";
    return 0;
  }
  ...
  // OPS_Get*() 会自动移动指针指向下一个参数
  int numData = 3;
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid integer (tag, iNode, jNode) in element Truss "
           << endln;
    return 0;
  }
  ...
  if (theUniaxialMaterial == 0) {
    opserr << "WARNING: Invalid material not found element Truss " << iData[0]
  ...
  theElement = new Truss(iData[0], ndm, iData[1], iData[2],
...
/**
 * @brief constructor of Truss
 * @param tag [in][int] element tag
 * @param dim [in][int] dimension number
 * @param Nd1 [in][int] node number
 * @param Nd2 [in][int] node number
 * @param theMat [in][UniaxialMaterial &] material object
 * @param a [in][double] section area
 * @param r [in][double] mass per unit length
 * @param damp [in][int] flag to include Rayleigh damping
 * @param cm [in][int] consistent mass flag
 */
Truss::Truss(int tag, int dim, int Nd1, int Nd2, UniaxialMaterial &theMat,
  ...
  if (theMaterial == 0) {
    opserr << "FATAL Truss::Truss - " << tag
           << "failed to get a copy of material with tag " << theMat.getTag()
           << endln;
    exit(-1);
  }
  ...
  for (int i = 0; i < 2; i++) theNodes[i] = 0;
...
/**
 * @brief invoked by a FEM_ObjectBroker - blank object that recvSelf needs to be
 * invoked upon
 * @param void
 */
Truss::Truss()
...
/**
 * @brief destructor delete must be invoked on any objects created by the object
 * and on the material object
 */
Truss::~Truss() {
  // invoke the destructor on any objects created by the object
  // that the object still holds a pointer to
  if (theMaterial != 0) delete theMaterial;
  if (theLoad != 0) delete theLoad;
  if (theLoadSens != 0) delete theLoadSens;
  if (initialDisp != 0) delete[] initialDisp;
}

/**
 * @brief get nodes number
 * @param void
 * @return 2
 */
int Truss::getNumExternalNodes(void) const { return 2; }

/**
 * @brief contains the tags of the end nodes
 * @param void
 * @return connectedExternalNodes
 */
const ID &Truss::getExternalNodes(void) { return connectedExternalNodes; }

/**
 * @brief get Node ptr
 * @param void
 * @return theNodes [Node**]
 */
Node **Truss::getNodePtrs(void) { return theNodes; }

/**
 * @brief get DOF number
 * @param void
 * @return numDOF [int]
 */
int Truss::getNumDOF(void) { return numDOF; }

/**
 * @brief to set a link to the enclosing Domain and to set the node pointers.
 * also determines the number of dof associated with the truss element, we set
 * matrix and vector pointers, allocate space for t(all of) matrix, determine
 * the length and set the transformation matrix.
 * @param theDomain [in][Domain *]
 */
void Truss::setDomain(Domain *theDomain) {
  ...
  */
  ...
  if ((theNodes[0] == 0) || (theNodes[1] == 0)) {
    if (theNodes[0] == 0)
      // Element <- Domain <- TaggedObject
      opserr << "Truss::setDomain() - truss" << this->getTag() << " node "
  ...
  if (dimension == 1 && dofNd1 == 1) {
    ...
    // shear and moment 4*4
    numDOF = 4;
    theMatrix = &trussM4;
    theVector = &trussV4;
  } else if (dimension == 2 && dofNd1 == 3) {
    // shear moment and axial 6*6
    numDOF = 6;
    theMatrix = &trussM6;
    theVector = &trussV6;
  } else if (dimension == 3 && dofNd1 == 3) {
    // 1 direction axial force, 1 direction shear force and 1 direction moment
    numDOF = 6;
    theMatrix = &trussM6;
    theVector = &trussV6;
  } else if (dimension == 3 && dofNd1 == 6) {
    // 3 direction force and 3 direction moment
    numDOF = 12;
    theMatrix = &trussM12;
    theVector = &trussV12;
  } else {
    opserr << "WARNING Truss::setDomain cannot handle " << dimension
           << " dofs at nodes in " << dofNd1 << " problem\n";
  ...
  if (dimension == 1) {
    ...
    if (L == 0.0) {
      opserr << "WARNING Truss::setDomain() - truss " << this->getTag()
             << " has zero length\n";
      return;
    }
...
int Truss::commitState() {
  ...
  // finished base class(element) stuff
  if ((retVal = this->Element::commitState()) != 0) {
    // return 0 On success
    opserr << "Truss::commitState () - failed in base class";
  }

  // finished Material state stuff
  retVal = theMaterial->commitState();
  // return 0 On success

  // all of commitState() return 0 in normal
  return retVal;
}

/**
 * @brief revert to last commit internal call Material::revertToLastCommit()
 * @param void
 * @return retVal
 */
...
/**
 * @brief update strain rate and setTrialStrain in material
 * @param void
 * @return TrialStrain Value
 */
int Truss::update(void) {
  // determine the current strain given trial displacements at nodes
  double strain = this->computeCurrentStrain();
  double rate = this->computeCurrentStrainRate();
  // Submit the strain and rate to material::setTrialStrain for calculation
  return theMaterial->setTrialStrain(strain, rate);
}

/**
 * @brief constructor stiffness matrix after transformation with call getTangent
 * @param void
 * @return stiff [Matrix &] pointer to static Matrix
 */
const Matrix &Truss::getTangentStiff(void) {
  if (L == 0.0) {
  ...
  // actually pointer to static Matrix
  ...
  //! constructor The stiffness matrix after transformation
...
/**
 * @brief constructor stiffness matrix after transformation with call
 * getInitialTangent
 * @param void
 * @return theMaterial [Matrix &] pointer to static Matrix
 */
const Matrix &Truss::getInitialStiff(void) {
  if (L == 0.0) {
    // - problem in setDomain() no further warnings
    theMatrix->Zero();
    return *theMatrix;
 ...
 * @return theMaterial [Matrix &] pointer to static Matrix
 */
const Matrix &Truss::getDamp(void) {
  if (L == 0.0) {
    // - problem in setDomain() no further warnings
    theMatrix->Zero();
    return *theMatrix;
  ...
  // get Element::getDamp() pointer to new theMatrix
  if (doRayleighDamping == 1) *theMatrix = this->Element::getDamp();
...
/**
 * @brief constructor stiffness matrix after transformation with rho and cMass
 * @param void
 * @return theMaterial [Matrix &] pointer to static Matrix
 */
const Matrix &Truss::getMass(void) {
  // zero the matrix
 ...
 * @return void
 */
void Truss::zeroLoad(void) { theLoad->Zero(); }

/**
 * @brief add load
 * @param theLoad [in][ElementalLoad *]
 * @param loadFactor [in][double]
 * @return void
 */
int Truss::addLoad(ElementalLoad *theLoad, double loadFactor) {
  opserr << "Truss::addLoad - load type unknown for truss with tag: "
         << this->getTag() << endln;
  return -1;
}

/**
 * @brief add Inertia Load to unbalance
 * @param accel [in][const Vector &]
 * @return int
 */
int Truss::addInertiaLoadToUnbalance(const Vector &accel) {
  // check for a quick return
  if (L == 0.0 || rho == 0.0) return 0;
...
/**
 * @brief add Inertia Load sensitivity to unbalance
 * @param accel [in][const Vector &]
 * @param somethingRandomInMotions [in][bool]
 * @return int
 */
...
/**
 * @brief get register force incream inertia
 * @param void
 * @return const Vector &
 */
...
/**
 * @brief send self message to recorder
 * @param commitTag [in][int]
 * @param theChannel [in][Channel &]
 * @return [int] return 0 on success
 */
int Truss::sendSelf(int commitTag, Channel &theChannel) {
  int res;
  ...
  // get rho flag
  data(6) = rho;
  // get Reylei damping flag
  data(7) = doRayleighDamping;
  // get cMass flag
  data(8) = cMass;

  // get material tag and get DBTag
  data(4) = theMaterial->getClassTag();
  int matDbTag = theMaterial->getDbTag();

  // get displacement of dimension
  if (initialDisp != 0) {
    for (int i = 0; i < dimension; i++) {
  ...
  if (matDbTag == 0) {
    ...
    if (matDbTag != 0) theMaterial->setDbTag(matDbTag);
  }
  data(5) = matDbTag;

  res = theChannel.sendVector(dataTag, commitTag, data);
  if (res < 0) {
    opserr << "WARNING Truss::sendSelf() - " << this->getTag()
...
/**
 * @brief truss creates a Vector, receives the Vector and then sets the internal
 * data with the data in the Vector
 * @param commitTag [in][int]
 * @param theChannel [in][channel &]
 * @param theBroker [in][FEM_ObjectBroker &]
 * @return [int] return 0 on success, error return data number
 */
int Truss::recvSelf(int commitTag, Channel &theChannel,
  ...
  for (int i = 0; i < dimension; i++) initialDisp[i] = 0.0;
...
/**
 * @brief display Self with text of another format
 * @param theViewer [in][Renderer &]
 * @param displayMode [in][int]
 * @param fact [in][float]
 * @param displayModes [in][const char **]
 * @param numModes [int]
 * @return [int] return 0 on success
 */
int Truss::displaySelf(Renderer &theViewer, int displayMode, float fact,
...
/**
 * @brief print message to OPS_Stream
 * @param s [in][OPS_Stream &]
 * @param flag [in][int]
 * @return void
 */
void Truss::Print(OPS_Stream &s, int flag) {
  ...
  if (flag == OPS_PRINT_CURRENTSTATE) {
    ...
    if (initialDisp != 0) {
      ...
      for (int i = 0; i < dimension; i++) s << initialDisp[i] << " ";
    }
...
/**
 * @brief computer current Strain, private and const function, call in updata()
 * @param void
 * @return [double] return Strain
 */
double Truss::computeCurrentStrain(void) const {
  // NOTE method will not be called if L == 0

  // determine the strain getTrialDisp
...
/**
 * @brief compute current strain rate, private and const function, call from
 * updata(), StrainRate = ε=dε/dt=(dl/dr) /l=u/l
 * @param void
 * @return [void] return strain rate
 */
double Truss::computeCurrentStrainRate(void) const {
  // NOTE method will not be called if L == 0

  // determine the strain getTrialVal
  const Vector &vel1 = theNodes[0]->getTrialVel();
  const Vector &vel2 = theNodes[1]->getTrialVel();

  double dLength = 0.0;
  for (int i = 0; i < dimension; i++) dLength += (vel2(i) - vel1(i)) * cosX[i];
...
/**
 * @brief set response
 * @param argv [in][const char **]
 * @param argc [in][int]
 * @param output [in][OPS_Stream &]
 * @return [Response *]
 */
Response *Truss::setResponse(const char **argv, int argc, OPS_Stream &output) {
  ...
  if ((strcmp(argv[0], "force") == 0) || (strcmp(argv[0], "forces") == 0) ||
    ...
    for (int i = 0; i < numDOFperNode; i++) {
      ...
      // every "P1_%d/0" will register to tag
...
/**
 * @brief get Response
 * @param responseID [in][int]
 * @param eleInfo [in][Information]
 * @return [int] return 0 on success
 */
int Truss::getResponse(int responseID, Information &eleInfo) {
  double strain, force;
  ...
  switch (responseID) {
    ...
    case 4:
      force = 0.0;
      if (L > 0.0) force = theMaterial->getTangent();
      // force = P / e
      // A * P = stress; L*e = strain;
      kVec(0, 0) = A * force / L;
...
// AddingSensitivity:BEGIN ///////////////////////////////////
/* int Truss::setParameter(const char **argv, int argc, Parameter &param) {
  if (argc < 1) return -1;
...
int Truss::updateParameter(int parameterID, Information &info) {
  switch (parameterID) {
    case 1:
      A = info.theDouble;
      return 0;
...
int Truss::activateParameter(int passedParameterID) {
...
const Matrix &Truss::getMassSensitivity(int gradNumber) {
  Matrix &mass = *theMatrix;
  mass.Zero();
...
const Vector &Truss::getResistingForceSensitivity(int gradNumber) {
  theVector->Zero();
...
int Truss::commitSensitivity(int gradNumber, int numGrads) {
  // Initial declarations
  int i;
  ...
  return theMaterial->commitSensitivity(strainSensitivity, gradNumber,
                                        numGrads);
} */
