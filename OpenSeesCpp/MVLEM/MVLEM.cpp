...
// (fibers) connected to rigid beams at the top and bottom levels, whereas the
// shear response is described via horizontal shear spring located at height ch
// from the bottom of the element, and is uncoupled from the flexural modeling
// parameters.
//
// References:
// 1) Vulcano A., Bertero V.V., and Colotti V. (1988). �Analytical Modeling of
// RC Structural Walls�, Proceedings, 9th World Conference on Earthquake
// Engineering, V. 6, Tokyo-Kyoto, Japan, pp. 41-46. 2) Orakcal K., Conte J.P.,
// and Wallace J.W. (2004). �Flexural Modeling of Reinforced Concrete Structural
// Walls - Model Attributes�, ACI Structural Journal, V. 101, No. 5, pp 688-698.
...
void *OPS_MVLEM(void) {
  ...
  if (theRho != 0) delete[] theRho;
  if (matTags != 0) delete[] matTags;

  if (theMaterialsConcrete != 0) delete[] theMaterialsConcrete;
  if (theMaterialsSteel != 0) delete[] theMaterialsSteel;
  if (theMaterialsShear != 0) delete[] theMaterialsShear;

  return theElement;
}

// typical constructor
MVLEM::MVLEM(int tag, double Dens, int Nd1, int Nd2,
             UniaxialMaterial **materialsConcrete,
             UniaxialMaterial **materialsSteel,
             UniaxialMaterial **materialsShear, double *Rho, double *thickness,
             double *width, int mm = 0, double cc = 0.0)

    : Element(tag, ELE_TAG_MVLEM),
      density(Dens),
      externalNodes(2),
      theMaterialsConcrete(0),
      theMaterialsSteel(0),
      theMaterialsShear(0),
      theLoad(0),
      MVLEMStrain(0),
  ...
  if (externalNodes.Size() != 2)
    opserr << "FATAL MVLEM::MVLEM() - out of memory, could not create an ID of "
              "size 2\n";
  externalNodes(0) = Nd1;
  externalNodes(1) = Nd2;
...
MVLEM::~MVLEM() {
  ...
  if (theMaterialsConcrete != 0) {
    for (int i = 0; i < m; i++)
      if (theMaterialsConcrete[i] != 0) delete theMaterialsConcrete[i];
    delete[] theMaterialsConcrete;
  }

  if (theMaterialsSteel != 0) {
    for (int i = 0; i < m; i++)
      if (theMaterialsSteel[i] != 0) delete theMaterialsSteel[i];
    delete[] theMaterialsSteel;
  }

  if (theMaterialsShear != 0) {
    for (int i = 0; i < 1; i++)
      if (theMaterialsShear[i] != 0) delete theMaterialsShear[i];
  ...
  if (Ec != 0) delete Ec;
  if (Es != 0) delete Es;
  if (stressC != 0) delete stressC;
  if (stressS != 0) delete stressS;
  if (MVLEMStrain != 0) delete MVLEMStrain;
}

int MVLEM::getNumExternalNodes(void) const { return 2; }

const ID &MVLEM::getExternalNodes(void) { return externalNodes; }

Node **MVLEM::getNodePtrs(void) { return theNodes; }

int MVLEM::getNumDOF(void) { return 6; }

void MVLEM::setDomain(Domain *theDomain) {
  ...
  if (theNodes[0] == 0) {
    opserr << "WARNING MVLEM::setDomain() - at MVLEM " << this->getTag()
  ...
  if (end1Crd.Size() != 2 && end2Crd.Size() != 2) {
    opserr << "MVLEM::setDomain(): 2 coords required at nodes, not enough "
              "provided for  element "
           << this->getTag();
    exit(-1);
  }
  ...
  {
    opserr << "MVLEM::setDomain(): 3 dof required at nodes, " << dofNd1
           << " and " << dofNd2 << " provided\n";
  }
...
int MVLEM::commitState() {
  int errCode = 0;

  // Commit Concrete material models
  for (int i = 0; i < m; i++) errCode += theMaterialsConcrete[i]->commitState();

  // Commit Steel material models
  for (int i = 0; i < m; i++) errCode += theMaterialsSteel[i]->commitState();

  // Commit Shear material models
  for (int i = 0; i < 1; i++) errCode += theMaterialsShear[i]->commitState();

  return errCode;
}

int MVLEM::revertToLastCommit() {
  int errCode = 0;
...
int MVLEM::revertToStart() {
  ...
  // Revert Steel material models
  for (int i = 0; i < m; i++) errCode += theMaterialsSteel[i]->revertToStart();

  // Revert Shear material model
  for (int i = 0; i < 1; i++) errCode += theMaterialsShear[i]->revertToStart();
...
const Matrix &MVLEM::getInitialStiff(void) {
...
const Matrix &MVLEM::getTangentStiff(void) {
  // Get material initial tangent
  for (int i = 0; i < m; ++i) {
...
void MVLEM::zeroLoad(void) {
  // does nothing - no elemental loads
}

int MVLEM::addLoad(ElementalLoad *theLoad, double loadFactor) { return 0; }

int MVLEM::addInertiaLoadToUnbalance(const Vector &accel) { return 0; }
...
const Vector &MVLEM::getResistingForce() {
  ...
  MVLEMR(0) =
      theMaterialsShear[0]
          ->getStress();  // get force from shear force-deformation relationship
...
int MVLEM::sendSelf(int commitTag, Channel &theChannel) {
  int res;
...
int MVLEM::recvSelf(int commitTag, Channel &theChannel,
  ...
  for (int i = 0; i < m; i++) {
    theMaterialsSteel[i] =
        theBroker.getNewUniaxialMaterial(matClassTags(i + m));
    if (theMaterialsSteel[i] == 0) {
      opserr << "MVLEM::recvSelf() - "
             << "failed to get blank uniaxial material.\n";
      return -3;
...
void MVLEM::Print(OPS_Stream &s, int flag) {
  if (flag == 0) {
    ...
    s << "  Global resisting force: " << this->getResistingForce() << endln
      << endln;
    ...
    for (int i = 0; i < m; i++) {
      s << "Fiber #: " << i + 1 << endln;
      s << "Concrete material with tag: " << theMaterialsConcrete[i]->getTag()
        << endln;
      theMaterialsConcrete[i]->Print(s, flag);

      s << "Steel material with tag: " << theMaterialsSteel[i]->getTag()
        << endln;
...
Response *MVLEM::setResponse(const char **argv, int argc, OPS_Stream &s) {
  ...
  // Fiber Strain
  else if (strcmp(argv[0], "Fiber_Strain") == 0 ||
           strcmp(argv[0], "fiber_strain") == 0) {
  ...
  else if (strcmp(argv[0], "Fiber_Stress_Concrete") == 0 ||
           strcmp(argv[0], "fiber_stress_concrete") == 0) {
  ...
  // Fiber Steel Stress
  else if (strcmp(argv[0], "Fiber_Stress_Steel") == 0 ||
           strcmp(argv[0], "fiber_stress_steel") == 0) {
  ...
  else if (strcmp(argv[0], "Shear_Force_Deformation") == 0 ||
           strcmp(argv[0], "shear_force_deformation") == 0) {
...
int MVLEM::getResponse(int responseID, Information &eleInfo) {
  switch (responseID) {
...
double *MVLEM::computeCurrentStrain(void) {
  const Vector &disp1 = theNodes[0]->getTrialDisp();
  const Vector &disp2 = theNodes[1]->getTrialDisp();

  MVLEMStrain[m] = disp1(0) - disp2(0) - c * h * disp1(2) -
                   (1 - c) * h * disp2(2);  // Shear deformation

  for (int i = 0; i < m; i++) {
    MVLEMStrain[i] =
        (-disp1(1) - x[i] * disp1(2) + disp2(1) + x[i] * disp2(2)) /
        h;  // Fiber (Flexural) Strains
  }
