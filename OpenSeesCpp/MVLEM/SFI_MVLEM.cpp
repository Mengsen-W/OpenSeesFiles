...
// University,
// Fullerton 								Kutay
// Orakcal Bogazici University, Istanbul, Turkey John Wallace
...
// fixed-strut angle approach (nDMaterial FSAM) into a two-dimensional
// fiber-based MVLEM model. The element generates automatically m internal nodes
// with 1 DOF at each macro-fiber (theNodesX with negative Tags) and adds them
// to the domain. These internal DOFs are used to enforce equilibrium equation
// sigmaX=0 at each element macro-fiber (RC panel) in order to complete its
// strain field; for details see referenced publications.
//
// References:
// 1) Kolozvari K., Orakcal K., and Wallace J. W. (2015). �Modeling of Cyclic
// Shear-Flexure Interaction in Reinforced Concrete Structural Walls. I:
// Theory�, ASCE Journal of Structural Engineering, 141(5), 04014135
...
void *OPS_SFI_MVLEM(void) {
  ...
  if (theThickness != 0) delete[] theThickness;
  if (theWidth != 0) delete[] theWidth;
  if (matTags != 0) delete[] matTags;

  if (theMaterials != 0) delete[] theMaterials;

  return theElement;
}

// Typical constructor
SFI_MVLEM::SFI_MVLEM(int tag, int Nd1, int Nd2, NDMaterial **materials,
                     double *thickness, double *width, int mm, double cc)
...

{
  ...
  P_6DOF.Zero();      // element force vector (6) - external nodes only used in
                      // globalForce recorder

  TotalMass = 0.0;
  NodeMass = 0.0;
  h = 0.0;

  // Check number of fibers - max is 999 to avoid overlapping in internal node
  // tags
  if (m > 999) {
    opserr << "WARNING: Number of fibers assigned is " << m
           << ". Maximum allowed number of fibers is 999!\n";
    exit(-1);
  }

  // Fill in the ID containing external node info with node id's
  if (externalNodes.Size() != 2 + m)
    opserr << "FATAL SFI_MVLEM::SFI_MVLEM() - out of memory, could not create "
              "an ID of size 2+m\n";
  ...
  // Calculate locations of concrete macro-fibers in the cross-section
  // (centerline - x = 0.0)
  x = new double[m];
  ...
  for (int i = 0; i < m; i++) {
    double sumb_i = 0.0;
    for (int j = 0; j < i + 1; j++) sumb_i += b[j];
...
SFI_MVLEM::SFI_MVLEM()
  ...
  SFI_MVLEMR.Zero();  // element force vector (6+m)
  P_6DOF.Zero();  // element force vector (6) - external nodes only - for global
                  // force recorder
...
SFI_MVLEM::~SFI_MVLEM() {
  ...
  if (theMaterial != 0) {
    for (int i = 0; i < m; i++)
      if (theMaterial[i] != 0) delete theMaterial[i];
    delete[] theMaterial;
  }
  ...
  if (Fy != 0) delete Fy;
  if (Fxy != 0) delete Fxy;
  if (Dens != 0) delete Dens;
  if (Dx != 0) delete Dx;
  if (Dy != 0) delete Dy;
  if (Dxy != 0) delete Dxy;
  if (SFI_MVLEMStrainX != 0) delete SFI_MVLEMStrainX;
  if (SFI_MVLEMStrainY != 0) delete SFI_MVLEMStrainY;
  if (SFI_MVLEMStrainXY != 0) delete SFI_MVLEMStrainXY;
  if (SFI_MVLEMStrain != 0) delete SFI_MVLEMStrain;
  if (theNodesX != 0) delete theNodesX;
  if (theNodesALL != 0) delete theNodesALL;
}

// Get number of nodes (external + internal)
int SFI_MVLEM::getNumExternalNodes(void) const { return 2 + m; }

// Get node tags
const ID &SFI_MVLEM::getExternalNodes(void) { return externalNodes; }

// Get shear deformation
double SFI_MVLEM::getShearDef(void) { return Dsh; }
...
void SFI_MVLEM::setDomain(Domain *theDomain) {
  ...
  // Get coordinates of end nodes - used for defining locations of internal
  // nodes
  ...
  if (h < 0.0) {
    opserr << "WARNING: Element height is negative. Define Nodes from bottom "
              "to top!";
    return;
  }
  ...
  for (int i = 0; i < m;
       i++) {  // Large NEGATIVE integer starting with tag of the element
    externalNodes(i + 2) =
        -(Nd1 * 1000 + i + 1);  // Max fibers is 999 to avoid overlap
  }
  ...
  for (int i = 0; i < m; i++) {
    ...
    theNode = new Node(nodeId_temp, 1, xLoc_temp,
                       yLoc_temp);  // create internal node with 1 DOF
    ...
    if (theDomain->addNode(theNode) ==
        false) {  // add internal node to the domain
  ...
  if (theNodes[0] == 0) {
    opserr << "WARNING SFI_MVLEM::setDomain() - at SFI_MVLEM " << this->getTag()
           << " node " << Nd1 << " does not exist in domain\n";
    return;  // Don't go any further - otherwise segemntation fault
  }
  if (theNodes[1] == 0) {
    opserr << "WARNING SFI_MVLEM::setDomain() - at SFI_MVLEM " << this->getTag()
           << " node " << Nd2 << " does not exist in domain\n";
    return;
  }

  // First ensure NodesX (internal - dummy) exist in Domain and set the node
  // pointers
  for (int i = 0; i < m; i++) {
    ...
    if (theNodesX[i] == 0) {
      opserr << "WARNING SFI_MVLEM::setDomain() - at SFI_MVLEM "
             << this->getTag() << " node " << NdX_temp1
  ...
  {
    opserr << "SFI_MVLEM::setDomain(): 3 dof required at nodes, " << dofNd1
           << " and " << dofNd2 << " provided\n";
  }
  ...
  for (int i = 0; i < m; i++) {
    ...
    {
      opserr << "SFI_MVLEM::setDomain(): 1 dof required at internal nodes, "
             << dofNdXi << " provided\n";
    }
  }
  ...
  // Create a vector to hop applied loads - NOT used in the current model
  // formulation (no element loads)
  if (theLoad == 0) theLoad = new Vector(6 + m);
  if (theLoad == 0) {
...
int SFI_MVLEM::commitState() {
  ...
  for (int i = 0; i < m; i++) errCode += theMaterial[i]->commitState();
...
int SFI_MVLEM::revertToLastCommit() {
  ...
  for (int i = 0; i < m; i++) errCode += theMaterial[i]->revertToLastCommit();
...
int SFI_MVLEM::revertToStart() {
  ...
  // Revert material models
  for (int i = 0; i < m; i++) errCode += theMaterial[i]->revertToStart();
...
int SFI_MVLEM::sendSelf(int commitTag, Channel &theChannel) {
  ...
  ID matClassTags(m);
  for (int i = 0; i < m; i++) matClassTags(i) = theMaterial[i]->getClassTag();
  res = theChannel.sendID(0, commitTag, matClassTags);

  // Send the material models
  for (int i = 0; i < m; i++) theMaterial[i]->sendSelf(commitTag, theChannel);
  return 0;
}

// Receive Self
int SFI_MVLEM::recvSelf(int commitTag, Channel &theChannel,
...
int SFI_MVLEM::displaySelf(Renderer &theViewer, int displayMode, float fact,
  ...
  {
    Matrix NodePLotCrds(
        m, 13);  // (panel id, x1,y1,z1, x2,y2,z2, x3,y3,z3, x4,y4,z4)

    // first set the quantity to be displayed at the nodes;
    // if displayMode is 1 through 3 we will plot material stresses otherwise
    // 0.0

    static Vector values(
        1);  // values to be plotted (either epsX, epsY, gammaXY)
...
void SFI_MVLEM::Print(OPS_Stream &s, int flag) {
  if (flag == 0) {
    ...
    s << "iNode: " << externalNodes(0) << ", jNode: " << externalNodes(1)
      << endln;
...

{
  ...
  // Shear deformation
  else if (strcmp(argv[0], "ShearDef") == 0 ||
           strcmp(argv[0], "sheardef") == 0) {
  ...
  // Element curvature
  else if (strcmp(argv[0], "Curvature") == 0 ||
           strcmp(argv[0], "curvature") == 0) {
  ...
  // Material output
    ...
    if (argc != 3) {
      opserr << "WARNING: Number of recorder input for RC Panel is: "
             << argc - 1
             << "; should be 2: panTag (one panel only: 1 to m) and "
                "$Response_Type.\n";
      return 0;
    }
    ...
    return theResponse = theMaterial[matNum - 1]->setResponse(&argv[argc - 1],
                                                              argc - 2, s);
  }
...
const Matrix &SFI_MVLEM::getInitialStiff(void) {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    SFI_MVLEMK(6 + i, 6 + i) =
        kx[i];  // Diagonal terms accounting for horizontal stiffness
  }
...
// Get current element tangent stiffness matrix from the material for the last
// updated strain
...
// N/A to this model - no element loads
int SFI_MVLEM::addLoad(ElementalLoad *theLoad, double loadFactor) { return 0; }

// N/A to this model - no element loads
int SFI_MVLEM::addInertiaLoadToUnbalance(const Vector &accel) { return 0; }
...
void SFI_MVLEM::computeCurrentStrain(void) {
  ...
  for (int i = 0; i < m; i++) {
    const Vector &dispXi =
        theNodesX[i]->getTrialDisp();  // 1 DOF at theNodesX - Vector of size 1
