...
/**
 * @brief To deal with tcl input and new object
 * @param void
 * @return [void*] return TwoNodeLink elment on success and nullptr on defeated
 */
void *OPS_TwoNodeLink() {
  ...
  while (OPS_GetNumRemainingInputArgs() > 0) {
    ...
    if (OPS_GetIntInput(&numdata, &mattag) < 0) {
      if (numArgs > OPS_GetNumRemainingInputArgs()) {
        // error move current arg back by one
  ...
  while (OPS_GetNumRemainingInputArgs() > 0) {
    ...
    if (strcmp(type, "-orient") == 0) {
      // if inputted x vector has error directly return nullptr;
      // if inputted y vector has any error, copy vector x to vector y;
      ...
      Mratio.resize(4);
      // To prevent 2-d genterate dirty data in Mratio[0] and Mratio[1]
      Mratio.Zero();
      numdata = 4;
      double *ptr = &Mratio(0);
      if (ndm == 2) {
...
/**`
 * @brief responsible for allocating the necessary space needed by each object
 * and storing the tags of the end nodes.
 * @param tag [int][in]
 * @param dim [int][in]
 * @param Nd1 [int][in]
 * @param Nd2 [int][in]
 * @param direction [const ID &] [in]
 * @param materials [UniaxialMaterial **] [in]
 * @param _y [const Vector][in]
 * @param _x [const Vector][in]
 * @param Mr [const Vector][in]
 * @param sdI [const Vector][in]
 * @param addRay [int][in]
 * @param m [double][in]
 */
TwoNodeLink::TwoNodeLink(int tag, int dim, int Nd1, int Nd2,
  ...
  if (connectedExternalNodes.Size() != 2) {
    opserr << "TwoNodeLink::TwoNodeLink() - element: " << this->getTag()
  ...
  for (int i = 0; i < numDIR; i++) {
    if ((*dir)(i) < 0 || (numDIM == 1 && (*dir)(i) > 0) ||
        (numDIM == 2 && (*dir)(i) > 2) || (numDIM == 3 && (*dir)(i) > 5)) {
      // if some error from input
      opserr << "TwoNodeLink::TwoNodeLink() - "
             << "incorrect direction " << (*dir)(i) << " is set to 0\n";
      (*dir)(i) = 0;
    }
  }
  ...
  // P-Delta moment contribution ratios
  // size of ratio vector is 2 for 2D-case and 4 for 3D-case
  if (Mratio.Size() == 4) {
    if (Mratio(0) < 0.0 || Mratio(1) < 0.0 || Mratio(2) < 0.0 ||
        Mratio(3) < 0.0) {
      opserr << "TwoNodeLink::TwoNodeLink() - "
  ...
  // size of ratio vector is 1 for 2D-case and 2 for 3D-case
...
/**
 * @brief delete must be invoked on any objects created by the object.
 * @param void
 */
TwoNodeLink::~TwoNodeLink() {
  ...
  if (theMaterials != 0) {
    for (int i = 0; i < numDIR; i++)
      if (theMaterials[i] != 0) delete theMaterials[i];
    delete[] theMaterials;
  }
}

/**
 * @brief get number External nodes, const function
 * @param void
 * @return 2
 */
int TwoNodeLink::getNumExternalNodes() const { return 2; }

/**
 * @brief get External nodes
 * @param void
 * @return [const ID &]
 */
const ID &TwoNodeLink::getExternalNodes() { return connectedExternalNodes; }

/**
 * @brief get Node pointers
 * @param void
 * @return [Node **]
 */
Node **TwoNodeLink::getNodePtrs() { return theNodes; }
...
void TwoNodeLink::setDomain(Domain *theDomain) {
  ...
  // set connectedExternalNodes in constructor
  int Nd1 = connectedExternalNodes(0);
  int Nd2 = connectedExternalNodes(1);
  // use tag of node get Node pointer
  ...
  if (!theNodes[0] || !theNodes[1]) {
    if (!theNodes[0]) {
      opserr << "TwoNodeLink::setDomain() - Nd1: " << Nd1
             << " does not exist in the model for ";
    } else {
      opserr << "TwoNodeLink::setDomain() - Nd2: " << Nd2
             << " does not exist in the model for ";
    }
  ...
  if (dofNd1 != dofNd2) {
    opserr << "TwoNodeLink::setDomain(): nodes " << Nd1 << " and " << Nd2
           << "have differing dof at ends for element: " << this->getTag()
           << endln;
    return;
  }
  ...
  if (numDIM == 1 && dofNd1 == 1) {
    ...
    opserr << "TwoNodeLink::setDomain() can not handle " << numDIM
...
/**
 * @brief commit state call Element::commitState() and Material::commitState()
 * @param void
...
int TwoNodeLink::commitState() {
  ...
  for (int i = 0; i < numDIR; i++) errCode += theMaterials[i]->commitState();
...
/**
 * @brief revert to last commit state, call Material::revertToLastCommit()
 * @param void
 * @return [int] 0 on success
 */
...
/**
 * @brief revert to start call Material::revertToStarts(), this function will be
 * call in constructor
 * @param void
 * @return [int] 0 on success
 */
int TwoNodeLink::revertToStart() {
  ...
  // il initial in constructor
  ub.Zero();
  ubdot.Zero();
  qb.Zero();

  // revert material models
  for (int i = 0; i < numDIR; i++) errCode += theMaterials[i]->revertToStart();
...
int TwoNodeLink::update() {
  ...
  // ugdot displacement velocity Vector
...
const Matrix &TwoNodeLink::getTangentStiff() {
  ...
  if (Mratio.Size() == 4) this->addPDeltaStiff(kl, qb);
...
/**
 * @brief get initial stiffness matrix
 * @param void
 * @return [const Matrix &]
 */
 ...
 * @return void
 */
void TwoNodeLink::zeroLoad() { theLoad->Zero(); }

/**
 * @brief add load to ElementLoad, for twoNodeLink element Don't allow add Load
 * any place to this elment
 * @param theLoad [ElementalLoad *]
 * @param loadFactor [double]
 * @return [int] 0 on success
 */
int TwoNodeLink::addLoad(ElementalLoad *theLoad, double loadFactor) {
  opserr << "TwoNodeLink::addLoad() - "
         << "load type unknown for element: " << this->getTag() << endln;

  return -1;
}

/**
 * @brief add InertialLoa to unbalance
...
/**
 * @brief get Resisting Force Vector
 * @param void
 * @return [const Vector &]
 */
const Vector &TwoNodeLink::getResistingForce() {
  ...
  // get resisting force
  for (int i = 0; i < numDIR; i++) qb(i) = theMaterials[i]->getStress();
  ...
  if (Mratio.Size() == 4) this->addPDeltaForces(ql, qb);
...
/**
 * @brief get Resisting Force Increase Initial
 * @param void
 * @return [const Vector&]
 */
...
/**
 * @brief send self to Channel
 * @param commitTag [int]
 * @param sChannel [Channel &]
 * @return [int] 0 On success, error return data number
 */
int TwoNodeLink::sendSelf(int commitTag, Channel &sChannel) {
  ...
  // send remaining data
  if (x.Size() == 3) sChannel.sendVector(0, commitTag, x);
  if (y.Size() == 3) sChannel.sendVector(0, commitTag, y);
  if (Mratio.Size() == 4) sChannel.sendVector(0, commitTag, Mratio);
  if (shearDistI.Size() == 2) sChannel.sendVector(0, commitTag, shearDistI);

  return 0;
}

/**
 * @brief receive Self data
 * @param commitTag [int]
 * @param rChannel [Channel &]
 * @param theBroker [FEM_ObjectBroker &]
 * @return [int] 0 on success, error return data number
 */
...
/**
 * @brief display Self with Renderer draw line
 ...
 */
int TwoNodeLink::displaySelf(Renderer &theViewer, int displayMode, float fact,
...
/**
 * @brief set response with OPS_Stream
 * @param argv [const char **]
 * @param argc [int]
 * @param output [OPS_Stream &]
 */
  ...
  // material output
    ...
    if (argc > 2) {
      ...
      if (matNum >= 1 && matNum <= numDIR)
        theResponse =
            theMaterials[matNum - 1]->setResponse(&argv[2], argc - 2, output);
    }
  }
...
/**
 * @brief get Response from element call
 * @param responseID [int]
 * @param eleInfo [Information &]
 * @return [int]
 */
int TwoNodeLink::getResponse(int responseID, Information &eleInfo) {
  ...
  switch (responseID) {
    ...
    case 2:  // local forces
      ...
      if (Mratio.Size() == 4) this->addPDeltaForces(*theVector, qb);
...
void TwoNodeLink::setUp() {
  ...
  // exist length Is greater than EPSILON
  if (L > DBL_EPSILON) {
    if (x.Size() == 0) {
      x.resize(3);
      x.Zero();
      x(0) = xp(0);
      if (xp.Size() > 1) x(1) = xp(1);
      if (xp.Size() > 2) x(2) = xp(2);
    } else if (onP0) {
...
/**
 * @brief set transformation matrix from global to local system
 */
...
/**
 * @brief set transformation matrix from local to basic system
 */
...
/**
 * @brief add pDelta forces
 * @param plocal [Vector &]
 * @param qBasic [const Vector &]
 * @return void
 */
...
int TwoNodeLink::setParameter(const char **argv, int argc, Parameter &param) {
  ...
  if (strcmp(argv[0], "material") == 0) {
    if (argc > 2) {
      int matNum = atoi(argv[1]);
      if (matNum >= 1 && matNum <= numDIR)
        return theMaterials[matNum - 1]->setParameter(&argv[2], argc - 2,
                                                      param);
    } else {
      return -1;
    }
