...
class TwoNodeLink : public Element {
 public:
  // constructors
  TwoNodeLink(int tag, int dimension, int Nd1, int Nd2, const ID &direction,
              UniaxialMaterial **theMaterials, const Vector y = 0,
              const Vector x = 0, const Vector Mratio = 0,
              const Vector shearDistI = 0, int addRayleigh = 0,
              double mass = 0.0);
  TwoNodeLink();
 ...
 private:
  ...
  Vector x;           // local orient x direction
  Vector y;           // local orient y direction
